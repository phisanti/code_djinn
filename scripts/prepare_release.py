...
def check_version_consistency():
    ...
    if setup_version and pyproject_version:
        if setup_version == pyproject_version:
            ...
            print(f"❌ Version mismatch: setup.py={setup_version}, pyproject.toml={pyproject_version}")
            return False
    else:
...
def main():
    ...
    all_checks_passed &= run_command(
        [sys.executable, os.path.join("scripts", "run_tests.py")],
        "Running unit tests"
    )
    
    # 3. Check package can be imported
    all_checks_passed &= run_command(
        [sys.executable, "-c", "import codedjinn; print('✅ Package imports successfully')"],
    ...
    run_command(["rm", "-rf", "dist/", "build/", "*.egg-info/"], "Cleaning build artifacts", check=False)
